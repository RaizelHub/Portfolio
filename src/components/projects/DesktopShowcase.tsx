...
const defaultCollabImages: ShowcaseImage[] = [
  {
    src: 'img/collab.png',
  ...
  return (
    <div className={`w-full space-y-2.5 ${className}`}>
      ...
      <div
        ...
        className={`group relative cursor-pointer rounded-xl overflow-hidden border border-[var(--border-subtle)] hover:border-[var(--border)] bg-[var(--surface-elevated)] shadow-md transition-all duration-300 ${aspectRatio} flex items-center justify-center`}
      >
        <AnimatePresence mode="wait">
          <motion.img
            ...
            className="w-full h-full object-cover object-top group-hover:scale-[1.015] transition-transform duration-300"
          />
        </AnimatePresence>
        ...
        {/* Dot Switcher Indicator (if multiple images) */}
        {images.length > 1 && (
          <div
            onClick={(e) => e.stopPropagation()}
            className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[var(--surface-elevated)]/90 backdrop-blur-md border border-[var(--border-subtle)] shadow-xs"
          >
            {images.map((_, i) => (
              <button
                key={i}
      ...
      {/* ── Subtitle and Tab Switcher Pill Strip (if enabled & >1 images) ── */}
      {showTabs && images.length > 1 && (
        <div className="flex items-center justify-between px-1 font-mono text-[11px] text-[var(--text-secondary)]">
          <span className="truncate max-w-[75%]">{currentImage.title}</span>
